...
Custom `oAuth2` implementation for working with ESI data in Google Workspace
...
graph TB;
  ...
  subgraph internet["The Internet"]
    direction TB;
    style internet fill:#fff,stroke:#eee,stroke-width:13px,color:#eee,border-radius:10px;

    subgraph ccpGamesServer["CCP Games' Server"]
      style ccpGamesServer fill:#ddd, stroke:#eee,stroke-width:13px
      subgraph oAuth2Server["oAuth2 Server"]
        style oAuth2Server fill:#ddd,stroke:#111,stroke-width:13px,color:black;
        eveSwaggerInterface["Eve Swagger Interface"];
        style eveSwaggerInterface fill:#113131,stroke:#113,stroke-width:1px,color:#111;
      end
    ...
    subgraph Google
      direction TB;
      style Google fill:#ddd,stroke:#eee,stroke-width:13px,color:black;
      subgraph appsScript["Google Apps Script"];
        style appsScript fill:#113,stroke:#eee,stroke-width:13px,color:#111;
        direction TB
        style appsScript fill:#ccc,stroke:#eee,stroke-width:13px;
        userProps[("User Properties")];

        subgraph oAuth2Client["oAuth2Client"]
          style oAuth2Client fill:#bbb, stroke:#eee, stroke-width:13px,color:#111;
          subgraph doGet["doGet()"]
            style doGet fill:#aaa,stroke:#eee,stroke-width:13px,color:#eee;
          ...
          subgraph tokenHandler["Token Handler"]
            style tokenHandler fill:#666, stroke: #777, stroke-width:13px,color:#eee;
            readStoredTokens -.->|Stale Token| renewToken;
            readStoredTokens -.-> |"Access Token"| requestEndpoint;
            style readStoredTokens fill:#33a,stroke:#111,stroke-width:3px,color:#eee;
            writeTokens[Write the Tokens to User Properties];
            style writeTokens fill:#33a,stroke:#111,stroke-width:3px,color:#eee;

            requestEndpoint["request restricted data"] --> |Access Token| eveSwaggerInterface;
            style requestEndpoint fill:#33a,stroke:#111,stroke-width:3px,color:#eee;
          end

          subgraph callback["Callback Function"]
            style callback fill:#aaa,stroke:#eee,stroke-width:13px,color:#111;
            successfulAuth{"Server Response\ncode is 200?"};
            style successfulAuth fill:#313,stroke:#111,stroke-width:3px,color:#eee;
            serverRedirect["The oAuth2 server is\nexpected to respond\nby making a http\n`POST` Request @ the\n**callback** *URI* and\nby sending both an\naccess token and a\nrenewal token in the\n*payload* "];
            style serverRedirect fill:#33a,stroke:#111,stroke-width:3px,color:#eee;
            parseTokens["Parse the tokens\nand note the time\n(they will expire in\ntwenty minutes)"];
            style parseTokens fill:#33a,stroke:#111,stroke-width:3px,color:#eee;
          end
        end
      end
      eveSwaggerInterface --> |"ESI Response"| parseEsiResponse;
      style eveSwaggerInterface fill:#33a,stroke:#111,stroke-width:3px,color:#eee;
      parseEsiResponse[Parse ESI Response] -->|"Parsed Data"| displayData;
      style parseEsiResponse fill:#33a,stroke:#111,stroke-width:3px,color:#eee;
      displayData ------> |The requested data| toApp
    end

    subgraph client["Client"]
      direction TB
      style client fill:#aaa,stroke:#eee,stroke-width:13px;
      clientInit["User Initiates\nAuthentication"];
      alreadyLogged{"Already Logged\nIn?"};
      style clientInit fill:#33a,stroke:#eee,stroke-width:3px,color:#eee;
      style alreadyLogged fill:#313,stroke:#aaa,stroke-width:3px,color:#eee;
